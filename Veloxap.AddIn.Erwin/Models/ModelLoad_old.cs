...
{
    internal class ModelLoad_old
    {
        private SCAPI.Application oApplication;
        private object oSelectedTreeNodeTag;

        public ModelLoad_old(ref SCAPI.Application oApp)
        {
            oApplication = oApp;


        }
        public ModelLoad_old(object oSelectedTreeNodeTag)
        {
            oApplication = new SCAPI.Application();
             
            this.oSelectedTreeNodeTag = oSelectedTreeNodeTag;
        }

        public ModelLoad_old( )
        {
            oApplication = new SCAPI.Application();
        ...
        public ModelInfo loadModel(SCAPI.PersistenceUnit oPersistenceUnit)
        {
            ...
            SCAPI.ModelObject oSelectedCollectionObject;
            ...
            if (oPersistenceUnit != null)
            {
                ...
                {
                    oSelectedCollectionObject = oSession.ModelObjects.Collect(oSession.ModelObjects.Root)[oSelectedTreeNodeTag];

                    ModelObject mModelObject = new ModelObject();

                    if (objectlist.Contains(oSelectedCollectionObject.ClassName))
                    {
                        mModelObject.setoObjectId(oSelectedCollectionObject.ObjectId);
                        mModelObject.setoClassName(oSelectedCollectionObject.ClassName);
                        mModelObject.setoName(oSelectedCollectionObject.Name);


                        // Model Object main properities (etc: D_Arac ve D_Arac özellikleri)
                        List<ObjectProperty> mObjectProperty = loadObjectProperities(false, oSelectedCollectionObject.ObjectId, oSession.ModelObjects.Root.ObjectId, oPersistenceUnit);


                        // D_Arac tablosunun sutunları ve sutunların özellikleri
                        mModelObject.setoModelObjects(loadSubModelObject(oSelectedCollectionObject.ObjectId, oPersistenceUnit));
