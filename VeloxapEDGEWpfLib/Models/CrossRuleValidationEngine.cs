...
namespace VeloxapEDGEWpfLib.Models
{
    ...
    internal sealed class BooleanExpressionParser
    {
        ...
        {
            if (_tokens.Count == 0)
                throw new Exception("Expression boş.");
        ...
        private BoolExprNode ParsePrimary()
        {
            ...
            {
                var expr = ParseOr();
                Expect(ExprTokenType.RParen, "Kapanış parantezi ')' bekleniyordu.");
                return expr;
            }
        ...
        {
            var scopeExpr = Expect(ExprTokenType.Identifier, "EXISTS sonrası scope bekleniyordu.").Value;
            Expect(ExprTokenType.LParen, "EXISTS sonrası '(' bekleniyordu.");
            var inner = ParseOr();
        ...
        {
            var scopeExpr = Expect(ExprTokenType.Identifier, "COUNT sonrası scope bekleniyordu.").Value;
            Expect(ExprTokenType.LParen, "COUNT sonrası '(' bekleniyordu.");
            var inner = ParseOr();
            Expect(ExprTokenType.RParen, "COUNT iç ifadesi için ')' bekleniyordu.");

            var countOperator = Expect(ExprTokenType.Operator, "COUNT sonrası sayı kıyas operatorü bekleniyordu.").Value;
        ...
        private ValueExprNode ParseValueExpr()
        {
            ...
            {
                Expect(ExprTokenType.LParen, "CONCAT sonrası '(' bekleniyordu.");
    ...
    internal static class CrossRuleParser
    {
        public static CrossRuleDefinition Parse(string ruleText)
        {
            if (string.IsNullOrWhiteSpace(ruleText))
                throw new ArgumentException("Rule text boş olamaz.");
            ...
            if (whenIndex != 0 || checkIndex < 0)
                throw new Exception("Rule formatı hatalı. Beklenen format: WHEN <TriggerNodeType> <WhenExpression> CHECK <CheckExpression>");
        ...
        private static void SplitWhenPart(string whenPartFull, out string triggerNodeType, out string whenExpressionText)
        {
            ...
            if (firstSpace <= 0 || firstSpace >= whenPartFull.Length - 1)
                throw new Exception("WHEN kısmı hatalı. Beklenen format: WHEN <TriggerNodeType> <WhenExpression>");
            ...
            if (string.IsNullOrWhiteSpace(triggerNodeType))
                throw new Exception("WHEN trigger node type boş olamaz.");

            if (string.IsNullOrWhiteSpace(whenExpressionText))
                throw new Exception("WHEN expression boş olamaz.");
        }
        ...
        private static RuleCheck ParseCheck(string text)
        {
            if (text.StartsWith("DESCENDANT(", StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (closeParen < 0)
                    throw new Exception("CHECK kısmı hatalı. DESCENDANT(...) kapanışı yok.");
                ...
                if (!rest.StartsWith(".", StringComparison.Ordinal))
                    throw new Exception("CHECK kısmı hatalı. Beklenen: DESCENDANT(Attribute).Property ... veya DESCENDANT(Attribute) ( ... )");
            ...
            if (text.StartsWith("PARENT(", StringComparison.OrdinalIgnoreCase))
            {
                int closeParen = text.IndexOf(')');
                if (closeParen < 0)
                    throw new Exception("CHECK kısmı hatalı. PARENT(...) kapanışı yok.");
                ...
                if (!rest.StartsWith(".", StringComparison.Ordinal))
                    throw new Exception("CHECK kısmı hatalı. Beklenen: PARENT(Entity).Property ... veya PARENT(Entity) ( ... )");
            ...
            if (text.StartsWith("ROOT(", StringComparison.OrdinalIgnoreCase))
            {
                int closeParen = text.IndexOf(')');
                if (closeParen < 0)
                    throw new Exception("CHECK kısmı hatalı. ROOT(...) kapanışı yok.");
                ...
                if (!rest.StartsWith(".", StringComparison.Ordinal))
                    throw new Exception("CHECK kısmı hatalı. Beklenen: ROOT(Model).Property ... veya ROOT(Model) ( ... )");
        ...
        private static BoolExprNode ParseGroupedRemainder(string text)
        {
            ...
            if (!text.StartsWith("(") || !text.EndsWith(")"))
                throw new Exception("Scope sonrası grup formatı hatalı.");
    ...
    internal static class CrossRuleCompiler
    {
            ...
            Func<string, NodeValueAccessor> accessorResolver)
        {
            if (expr is ComparisonExprNode cmp)
            {
                ...
                {
                    // rightEvaluator runtime value olabilir, yine de regex cache kullanıyoruz
                    return node =>
                    {
        ...
        private static Func<ValidationNode, bool> BuildExistsPredicate(
            ...
            Func<string, NodeValueAccessor> accessorResolver)
        {
            // OPT-2 ve OPT-3: inner predicate ve scope resolver compile-time hazırlanır
        ...
        private static Func<ValidationNode, IReadOnlyList<ValidationNode>> BuildScopeResolver(string scopeExpression)
        {
            if (scopeExpression.StartsWith("DESCENDANT(", StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (closeParen < 0)
                    throw new Exception("Scope expression hatalı: " + scopeExpression);
        ...
        private static NodeValueAccessor ResolveOperandAccessor(string leftOperand)
        {
            ...
            if (leftOperand.StartsWith("PARENT(", StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (closeParen < 0)
                    throw new Exception("Operand PARENT(...) hatalı: " + leftOperand);
                ...
                if (!rest.StartsWith(".", StringComparison.Ordinal))
                    throw new Exception("Operand PARENT(...) formatı hatalı: " + leftOperand);
            ...
            if (leftOperand.StartsWith("ROOT(", StringComparison.OrdinalIgnoreCase))
            {
                int closeParen = leftOperand.IndexOf(')');
                if (closeParen < 0)
                    throw new Exception("Operand ROOT(...) hatalı: " + leftOperand);

                string rootNodeType = leftOperand.Substring("ROOT(".Length, closeParen - "ROOT(".Length).Trim();
                string rest = leftOperand.Substring(closeParen + 1).Trim();

                if (!rest.StartsWith(".", StringComparison.Ordinal))
                    throw new Exception("Operand ROOT(...) formatı hatalı: " + leftOperand);
            ...
            if (leftOperand.StartsWith("DESCENDANT(", StringComparison.OrdinalIgnoreCase))
            {
                int closeParen = leftOperand.IndexOf(')');
                if (closeParen < 0)
                    throw new Exception("Operand DESCENDANT(...) hatalı: " + leftOperand);

                string descendantNodeType = leftOperand.Substring("DESCENDANT(".Length, closeParen - "DESCENDANT(".Length).Trim();
                string rest = leftOperand.Substring(closeParen + 1).Trim();

                if (!rest.StartsWith(".", StringComparison.Ordinal))
                    throw new Exception("Operand DESCENDANT(...) formatı hatalı: " + leftOperand);
                ...
                foreach (var target in targets)
                {
                    ...
                    if (!rule.CheckPredicate(target))
                    {
                        ...
                        issues.Add(new CrossValidationIssue
                        {
                            ...
                            Message = "Cross kural sağlanmadı."
                        });
                    }
                }
            }
